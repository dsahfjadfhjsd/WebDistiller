...
def run_evaluation(
    ...
    if task_type == 'code':
        ...
        for item, input_prompt, result in tqdm(zip(filtered_data, input_list, output_list), total=len(input_list)):
            ...
            elif 'Correct_Answer' in item and item['Correct_Answer'] is not None:
                labeled_answer = item['Correct_Answer']
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate model outputs (WebThinker style).")
    ...
    parser.add_argument('--api_key', type=str, default='empty', help='API key for LLM')
    ...
    run_evaluation(
        ...
        output_metrics_path=os.path.basename(output_metrics_path),
        output_metrics_overall_path=os.path.basename(output_metrics_overall_path),
