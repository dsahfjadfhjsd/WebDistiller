...
class MemoryManager:
    ...
    @property
    def episode_memories(self) -> List[str]:
        """Alias for factual_memories (backward compatibility)."""
        return self.factual_memories

    @episode_memories.setter
    def episode_memories(self, value: List[str]):
        self.factual_memories = value

    @property
    def working_memory(self) -> Optional[str]:
        """Alias for procedural_memories (backward compatibility)."""
        return self.procedural_memories

    @working_memory.setter
    def working_memory(self, value: Optional[str]):
        self.procedural_memories = value

    @property
    def tool_memory(self) -> Optional[str]:
        """Alias for experiential_memories (backward compatibility)."""
        return self.experiential_memories

    @tool_memory.setter
    def tool_memory(self, value: Optional[str]):
        self.experiential_memories = value
    ...
    def add_fold(
        ...
        """Add a memory fold (legacy text-based interface)."""
        self.factual_memories.append(episode_memory)
        self.procedural_memories = working_memory
        self.experiential_memories = tool_memory
        self.fold_count += 1

        self.interactions.append({
            "type": "memory_folding",
            "fold_number": self.fold_count,
            "factual_memory": episode_memory,
            "procedural_memory": working_memory,
            "experiential_memory": tool_memory,
        })

    def get_memory_summary(self) -> str:
        """Get formatted memory summary for system prompt."""
        ...
        if self.factual_memory:
            ...
            for key, entry in list(self.factual_memory.items())[:10]:
                ...
                if current:
                    parts.append(f"  {entry.entity_id}.{entry.attr} = {current}")
            parts.append("")

        # Legacy episode memory (most recent)
        if self.factual_memories:
            parts.append("== Episode Memory ==")
