...
async def reasoning_loop(
    ...
    async def _apply_memory_fold(reason: str) -> bool:
        ...
        # Cooldown: skip folding for Δ steps after a fold
        ...
        # Parse factual candidates from folded memory and add to structured memory
        try:
        ...
        # Parse procedural trace and add nodes
        try:
        ...
        # Build context pack for the Reasoner (Eq.4)
        current_subgoal = question  # Use question as default subgoal
        try:
            proc_data = json.loads(procedural_memory) if procedural_memory.strip().startswith('{') else {}
        ...
        # Use context pack if available, otherwise fall back to memory_text
    ...
    async def _detect_milestone(
        ...
        try:
            ...
            # Format memory state summary for the milestone detector
        ...
        if markers["BEGIN_TOOL_CALL"] in response:
            ...
            if tool_call_text:
                try:
                    ...
                    # Synthesize ExtractionIntent before tool call (Paper Section 3.3)
                    # i_t = G_φ^Intent(q, τ_t, a_t)
                    # Infer current subgoal from: procedural memory > tool args > question
                    current_subgoal = question  # Default subgoal
                    ...
                    # Override with explicit goal from tool args if present
                    if tool_args.get("goal"):
                        ...
                        # For search, the query IS the subgoal
                    ...
                    if getattr(args, 'extraction_intent_enabled', True):
                        if getattr(args, 'use_llm_synthesis', False) and aux_client:
                            # Paper-aligned: G_φ^Intent via Manager LLM
                            ...
                            # Rule-based fallback (fast, no LLM cost)
                    ...
                    # Intent-driven Cognitive Distillation (replaces generic auto-summarize)
                    ...
                    if (
                        ...
                        # No intent available, just truncate oversized results
                        tool_result = tool_result[:20000] + f"\n\n[Content truncated from {len(tool_result)} chars]"

                    # Structured memory updates — always after every tool call
                    # Parse candidates from distillation into factual memory
                    ...
                    # Add procedural node for every tool call (M^P trace)
                    ...
                    if (
                        ...
                        and state["tool_call_count"] >= 3  # Only check after enough context
                    ):
                    ...
                    # Capacity-based folding trigger
                    if (
                        memory_manager.can_fold()
                        and fold_threshold
                        and state["total_tokens"] >= MAX_TOKENS * fold_threshold
...
async def run_single_question(
    ...
    # Read from reasoner/manager config keys (with fallback to legacy main_model/auxiliary_model)
    ...
    try:
        ...
        class Args:
            def __init__(self):
                ...
                # Paper-aligned memory enable flags (with fallback to legacy)
                ...
                # Model names from reasoner/manager config
                ...
                # Active Cognitive Distillation configuration
                ...
                # Cooldown, tail, milestone from config
                ...
                # Extraction Intent configuration (Paper Section 3.3)
