...
Output only the relevant information without explanation:"""
    ...
    async def call_tool(
        ...
        try:
            if tool_name == "web_search":
                return await self._web_search(arguments, reasoning_context)
            elif tool_name == "browse_pages":
                return await self._browse_pages(arguments)
            elif tool_name == "click_link":
                return await self._click_link(arguments, reasoning_context)
    ...
    async def _web_search(self, args: dict, reasoning_context: str = "") -> str:
        ...
        search_intent = ""
        if self.aux_client and reasoning_context:
            try:
    ...
    async def _browse_pages(self, args: dict) -> str:
        ...
        for url in urls:
            if url in cached_results:
                ...
                if len(extracted_text) > MAX_PER_URL_CHARS:
                    if query:
                        # Try compression first if we have a query
                ...
                elif query and len(extracted_text) > 6000:
                    # Medium-sized content with query: compress
                    compressed = self.compressor.compress_content(
                        content=extracted_text,
                        query=query,
                        max_length=6000,
    ...
    async def _click_link(self, args: dict, reasoning_context: str = "") -> str:
        ...
        if not goal and self.aux_client and reasoning_context:
            try:
                ...
                if click_intent:
                    goal = click_intent
                    print(f"🎯 Click intent: {goal}")
            except Exception as e:
                                   
                print(f"⚠️ Click intent analysis failed: {e}")
